"use client";
import { useReceiptWizard } from "@/app/(dashboard)/hooks/use-receipt-wizard";
import { getReceiptData } from "@/app/(dashboard)/lib/get-document-data";
...
export default function ReceiptFormWizard({
  open,
  onOpenChange,
  onSuccess,
  order,
  ...
  const {
    ...
    initialData: getReceiptData(order),
    onSuccess,
  });

  const isEditMode = !!order.mappedDocuments.RECEIPT;
  const invoiceId = order.mappedDocuments.RECEIPT?.id;
        ...
        <div className="flex h-full">
          ...
          <div className="hidden w-96 flex-col bg-gray-900 p-6 text-primary-foreground md:flex max-h-screen overflow-y-auto">
            <h2 className="text-xl font-semibold mb-5">
              {!isEditMode ? "Create Receipt" : "Edit Receipt"}
            </h2>

            <div className="flex-1 space-y-1">
              {receiptTabs.map((tab, index) => {
              ...
              <div className="flex items-center justify-between border-b px-8 py-4">
                ...
                <div className="hidden md:block">
                  <h3 className="text-lg font-semibold">
                    {receiptTabs[currentTabIndex]?.label}
                  </h3>
                </div>
              </div>
