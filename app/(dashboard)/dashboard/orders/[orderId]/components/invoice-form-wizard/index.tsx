"use client";
import { useInvoiceWizard } from "@/app/(dashboard)/hooks/use-invoice-wizard";
import { getInvoiceData } from "@/app/(dashboard)/lib/get-document-data";
...
import { InvoiceNotesStep } from "./note-step";
import { PaymentInstructionsStep } from "./payment-instruction-step";
import { PaymentStatusStep } from "./payment-status-step";
...
const invoiceTabs = [
  ...
  { id: "paymentInstructions", label: "Payment Instructions" },
...
export default function InvoiceFormWizard({
  open,
  onOpenChange,
  onSuccess,
  order,
  ...
  const {
    ...
    initialData: getInvoiceData(order),
    onSuccess,
  });

  const isEditMode = !!order.mappedDocuments.INVOICE;
  const invoiceId = order.mappedDocuments.INVOICE?.id;
        ...
        <div className="flex h-full">
          ...
          <div className="hidden w-96 flex-col bg-gray-900 p-6 text-primary-foreground md:flex max-h-screen overflow-y-auto">
            <h2 className="text-xl font-semibold mb-5">
              {!isEditMode ? "Create Invoice" : "Edit Invoice"}
            </h2>
            ...
            >
              ...
              <div className="flex items-center justify-between border-b px-8 py-4">
                ...
                <div className="hidden md:block">
                  <h3 className="text-lg font-semibold">
                    {invoiceTabs[currentTabIndex]?.label}
                  </h3>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto flex flex-col px-4 py-6 md:px-8">
                <Tabs value={currentTab.id} className="w-full ">
                  ...
                  <TabsContent value="paymentInstructions">
                    <PaymentInstructionsStep form={form} />
                  </TabsContent>

                  <TabsContent value="paymentStatus">
                    <PaymentStatusStep form={form} />
                  </TabsContent>
