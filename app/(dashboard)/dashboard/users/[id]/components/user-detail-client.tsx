...
export function UserDetailClient({ userId }: UserDetailClientProps) {
  ...
  const updateRoleFn = async (userId: string, newRole: Role) => {
    const { data, error } = await authClient.admin.setRole({
      userId,
      role: newRole,
    });

    // 3. Return a structure that matches your mutation's onSuccess logic
    if (error) {
  ...
  const updateRoleMutation = useMutation({
    mutationFn: (newRole: Role) => updateRoleFn(userId, newRole),
    onSuccess: (result) => {
      if (result.success) {
  ...
  const banUserFn = async (
    ...
    // 1. Convert Date object to "seconds from now" (duration)
    let durationInSeconds = undefined;
    if (expiresAt) {
      const now = new Date();
      const diff = expiresAt.getTime() - now.getTime();
      // Ensure we don't send negative numbers
      durationInSeconds = diff > 0 ? Math.floor(diff / 1000) : undefined;
    }
    ...
    const { data, error } = await authClient.admin.banUser({
      userId: userId,
      banReason: reason,
      banExpiresIn: durationInSeconds, // Pass the calculated duration
    });
  ...
  const unbanUserFn = async (userId: string) => {
    const { data, error } = await authClient.admin.unbanUser({
      userId: userId,
    });

    if (error) {
      // Return a failed status so your onSuccess logic handles it
      return { success: false, error: error.message };
    }
  ...
  const unbanMutation = useMutation({
    ...
    // This catches unexpected crashes (like network failures)
